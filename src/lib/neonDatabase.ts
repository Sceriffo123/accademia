...
export async function getTableRecords(tableName: string, limit: number = 100): Promise<any[]> {
  try {
    ...
    const allowedTables = ['users', 'normatives', 'documents', 'activity_logs', 'course_enrollments', 'course_modules', 'courses', 'user_role_overrides', 'permissions', 'sections', 'role_permissions', 'role_sections', 'roles', 'quizzes'];
